...
let
  ...
  # from the ro mount. The system prompt template is NOT baked in here anymore
  # — linear-agent (prompt.go) go:embeds it and ships the fully-rendered
  # document per dispatch instead, see the entrypoint's .system/.prompt read.
  ...
  entrypoint = ''
    ...
    # The receiver (linear-agent's prompt.go/nomad.go) already resolved this
    # session's trigger + thread against the Linear API and assembled both
    # halves of what pi needs — this worker no longer parses Linear's webhook
    # shape at all, just two fields it invented itself:
    #
    #   .system  the static operating manual + frontmatter, ready for
    #            --append-system-prompt as-is
    #   .prompt  frontmatter + thread + the triggering request, ready for
    #            the RPC prompt message as-is
    #
    # Fail loudly rather than let a missing/foreign shape (e.g. a stale worker
    # image talking to a receiver that changed the payload contract) silently
    # hand pi an empty prompt.
    jq -r '.system // ""' /local/webhook.json > /local/system.md
    prompt=$(jq -r '.prompt // ""' /local/webhook.json)
    if [ ! -s /local/system.md ] || [ -z "$prompt" ]; then
      fail "dispatch payload is missing .system or .prompt — this worker only understands linear-agent's {system, prompt} shape."
    fi
    ...
    LD_LIBRARY_PATH="${pkgs.glibc}/lib" /opt/pi/pi --mode rpc \
      --append-system-prompt /local/system.md \
